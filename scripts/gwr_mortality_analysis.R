...
gwr.bw <- bw.gwr(Deceased ~ PM25+population+Humid+Tested+Temp+PM10+Confirmed+AQIUS ,
                 data = SPDF,
                 approach = "AICc",
                 kernel='bisquare',
                 adaptive=TRUE)
...
gwr.res <-gwr.basic(Deceased ~ PM25+population+Humid+Tested+Temp+PM10+Confirmed+AQIUS,
                     data=SPDF,
                     bw=gwr.bw,
                     kernel='bisquare',
                     adaptive=TRUE)
...
state_map@data$est_Conf <- gwr.res$SDF$Confirmed
...
state_map@data$t_Conf <- gwr.res$SDF$Confirmed_TV
...
est_temp<-spplot(state_map,"est_Temp", main = "Temprature", 
                 ...
                 col="transparent",
                 col.regions=col.palette(100))
est_conf <- spplot(state_map, "est_Conf", main = "Confirmed cases",
                   sp.layout=list(polys),
                   col="transparent",
                   col.regions=col.palette(100))
...
grid.arrange(est_pop,est_pm25,est_pm10, est_hum, est_temp, est_conf, est_test,ncol= 3, top = textGrob("Local Estimates",gp=gpar(fontsize=25)))
...
t_Temp <- spplot(state_map, "t_Temp", main="Temprature",
                 ...
                 col="transparent",
                 col.regions=rev(col.palette.t(100)))
t_conf <- spplot(state_map, "t_Conf", main="Confirmed Cases",
                 sp.layout=list(polys),
                 col="transparent",
                 col.regions=rev(col.palette.t(100)))
...
grid.arrange(t_pop, t_pm25, t_pm10, t_hum, t_Temp, t_conf, t_test,ncol=3,
